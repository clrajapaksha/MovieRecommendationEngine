...
public class RecommendationEngine {
	...
	public ArrayList<Recommendation> showRecommendations(String name){
        ...
        //Display in window
        //method to show n recommendations for each entity
	public void showTopRecommendationsInWindow(){
        ...
        public ArrayList<String> getMovieNames(){
            ...
            for(int id: rec.keySet()){
               list.add(dict.get(id));
            }
            
            return list;
        }
        
        public String[] getRecommendationList(String name){
            /*
            HashMap<Integer,String> dict = this.handler.getMappingDictionary();
            
            ArrayList<Recommendation> listOfRec = rec.get(1);
            Collections.sort(listOfRec,new RecommendationComparator());
            
            
            String[] recList = new String[topN];
            for (int i = 0; i < listOfRec.size(); i++) {
					if(i==topN)
						break;
					Recommendation r = listOfRec.get(i);
            ...
            recList[i]=((i+1)+". "+recName+" ["+r.getRecId()+"] "+r.getcValue());
                System.out.println("### : " + recName);
            }
            return recList;
            */
            String[] recomendations = new String[topN];
            int id = this.handler.getReverseMappingDictionary().get(name);
				HashMap<Integer,String> dict = this.handler.getMappingDictionary();
				...
				for (int i = 0; i < listOfRec.size(); i++) {
					...
					Recommendation r = listOfRec.get(i);
					String recName = dict.get(r.getRecId());
					recomendations[i]=((i+1)+". "+recName+" ["+r.getRecId()+"] "+r.getcValue());
				}
				System.out.println();
